...
class TestDashboardEndToEndWorkflow:
    ...
    def test_full_dashboard_load_with_both_services(
        self, mock_sonarr_missing, mock_sonarr_count, mock_sonarr_status,
        mock_radarr_missing, mock_radarr_count, mock_radarr_status,
        app, client, temp_instance_path
    ):
        """Test complete workflow: configure both services → load dashboard → verify all stats."""
        # Mock Radarr
        mock_radarr_status.return_value = {'version': '4.5.2.7388', 'instanceName': 'Radarr'}
        mock_radarr_count.return_value = 150
        mock_radarr_missing.return_value = 12

        # Mock Sonarr
        mock_sonarr_status.return_value = {'version': '3.0.10.1567', 'instanceName': 'Sonarr'}
