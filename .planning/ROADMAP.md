...
**Goal:** Add username/password authentication to protect the application

**Deliverable:** Single-user authentication system with login page, session management, and protected routes

**Plans:** 3 plans in 2 waves

Plans:
- [x] 11-01-PLAN.md -- Core auth system (User model, Flask-Login, auth routes, login/setup templates)
- [x] 11-02-PLAN.md -- Route protection, nav bar update, AJAX 401 handler, password change, CLI reset
- [x] 11-03-PLAN.md -- Auth test suite and auth-aware test fixtures

**Status:** Complete (2026-02-14) - 3/3 plans + bug fixes
...
- Initial user setup on first run
- Password change on Settings page
- CLI password reset command

**Verification:**
- Unauthenticated users are redirected to login page
- Valid credentials grant access to protected pages
- Invalid credentials show error message
- Session persists across page navigation
- Logout clears session and redirects to login
- Password is securely hashed in database
- Dashboard remains publicly viewable without authentication
...
### Phase 13: Tailwind CSS Local Installation

**Goal:** Replace Tailwind CDN with local PostCSS/CLI build to eliminate production warning and improve reliability

**Source:** Browser console warning: "cdn.tailwindcss.com should not be used in production"

**Deliverable:** Tailwind CSS compiled locally via PostCSS or CLI, CDN script tag removed, all styles preserved
...
Plans:
- [ ] 13-01-PLAN.md — Install pytailwindcss, configure Tailwind, compile CSS, update templates
- [ ] 13-02-PLAN.md — Update CSP, create build script, verify visual correctness

**Scope:**
- Install Tailwind CSS CLI via pytailwindcss (no Node.js required)
- Configure tailwind.config.js with content paths for Jinja templates and JS
- Generate production CSS bundle (minified)
- Replace CDN script with compiled CSS link in all 3 templates (base.html, login.html, setup.html)
- Update CSP to remove cdn.tailwindcss.com allowlist
- Create developer build script for CSS regeneration

**Verification:**
- No CDN script tags in base.html, login.html, or setup.html
- Browser console shows no Tailwind warnings
- All pages render with correct styling (dark mode, responsive, components)
- CSS bundle is minified for production
- CSP script-src contains only 'self' (no external CDN)

---

### Phase 13.2: Footer version number and GitHub link (Complete)

**Goal:** Replace copyright notice in footer with version number and GitHub repository link
**Depends on:** Phase 13.1

**Deliverable:** Footer displays app version (centered) with GitHub logo/link pointing to https://github.com/fisherd80/Listarr, dark mode toggle remains on right side

**Status:** Complete (2026-02-18) - direct implementation (no plan)

**Verification:**
- Footer shows "v1.0.0" centered
- GitHub logo links to https://github.com/fisherd80/Listarr
- Dark mode toggle remains on right side
- No copyright text

---

### Phase 13.1: Dark mode toggle on bottom banner (Complete)

**Goal:** Add manual dark mode toggle button to footer with localStorage persistence and FOUC prevention
**Depends on:** Phase 13

**Requirements:** [DM-01, DM-02, DM-03]

**Deliverable:** Users can toggle between light and dark mode via a footer button, with preference persisting across sessions and no flash of wrong theme on any page

**Status:** Complete (2026-02-18) - 1/1 plans executed
- 13.1-01: Class-based dark mode, FOUC prevention script, footer toggle button

Plans:
- [x] 13.1-01-PLAN.md — Switch to class-based dark mode, add FOUC script, add toggle button to footer

**Verification:**
- Toggle button visible in footer on all pages extending base.html
- Clicking toggle switches theme instantly without page reload
- Preference persists across page loads via localStorage
- No FOUC on any page (base, login, setup)
- System preference used as fallback when no stored preference

---

### Phase 14: Release Hardening (Complete)

**Goal:** Code cleanup, error handling, and Docker hardening to bring the codebase to production quality

**Depends on:** Phase 13.2

**Deliverable:** Clean codebase with no dead code or AI slop, centralized error handling with custom error pages, production-ready Docker configuration with non-root user and fresh-volume boot support

**Status:** Complete (2026-02-18) - 3/3 plans executed
- 14-01: Centralized error handling (404, 500, CSRF, Exception), standalone error pages, JSON standardization
- 14-02: Docker polish (healthcheck in compose, TZ/LOG_LEVEL env vars, container_name)
- 14-03: Dead code removal (debug endpoint, wrapper functions, wildcard imports, redundant imports, section dividers), bare exception fix, test updates

Plans:
- [x] 14-01-PLAN.md — Centralized error handling, standalone error pages, JSON standardization
- [x] 14-02-PLAN.md — Docker polish (docker-compose.yml, Dockerfile healthcheck)
- [x] 14-03-PLAN.md — Dead code removal, pattern simplification, test fixes

**Scope:**
- Thorough dead code and AI slop removal (production code + tests)
- Centralized Flask error handler (404, 500, generic Exception)
- Custom error pages (minimal branded, dark mode support)
- Standardized JSON error responses across all API endpoints
- Docker non-root user configuration
- Production-ready docker-compose.yml (volumes, env vars, restart, healthcheck)
- Fresh-volume boot path validation (empty volume → /setup flow)
- Startup validation audit against public release standards
- Test regeneration if coverage drops after cleanup

**Verification:**
- No commented-out blocks, unused imports, or debug prints remain
- No redundant try/except or over-verbose AI-generated boilerplate
- Custom 404 page shows branded error with "Go to Dashboard" link
- Custom 500 page shows friendly error, no tracebacks exposed
- All API endpoints return `{"success": false, "message": "..."}` on errors
- Docker container runs as non-root user
- docker-compose.yml works from fresh clone with empty volume
- App redirects to /setup on first run with empty database
- All tests pass after cleanup (regenerate if needed)

---

### Phase 14.1: Complete Test Suite Regeneration (INSERTED)
...
Plans:
- [ ] 14.1-01-PLAN.md — Regenerate dashboard, jobs, and settings route tests (delete test_dashboard_auth.py)
- [ ] 14.1-02-PLAN.md — Regenerate lists route tests and create new schedule route tests
- [ ] 14.1-03-PLAN.md — Regenerate auth route tests and full suite validation

### Phase 15: Documentation & Release

**Goal:** Public-facing documentation, CI/CD pipeline, and release process to ship v1.0.0

**Depends on:** Phase 14

**Deliverable:** Complete README for public audience, GitHub Actions CI/CD pipeline with Docker Hub publishing, and successful v1.0.0 release via develop → main merge

**Scope:**
- Full README.md rewrite for public audience (project description, features, Docker install, env vars, first-run steps, known limitations)
- Screenshot placeholders for key pages (Dashboard, Lists, Wizard, Jobs) — dark mode only
- GitHub Actions CI/CD pipeline (test + build Docker image)
  - Triggers: version tags (build + push to Docker Hub) and develop pushes (test only)
  - Registry: Docker Hub (docker.io/fisherd80/listarr)
- Release merge strategy (develop → main, release branch, v1.0.0 tag)
- GitHub release notes
- CHANGELOG.md (Claude's discretion on format)

**Verification:**
- README suitable for public GitHub audience (not a development log)
- Screenshot sections present with placeholders
- GitHub Actions workflow runs tests on develop push
- GitHub Actions builds and pushes Docker image on version tag
- Docker Hub has listarr image available
- develop → main merge completes cleanly
...
- User authentication protects the application
- Security hardened for production
- Codebase hardened for production (no dead code, proper error handling, Docker ready)
- Release-ready with complete documentation and CI/CD pipeline
...
*Phases: 15 (13 complete + 8 sub-phases, 2 remaining)*
